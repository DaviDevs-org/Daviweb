...
export class PaymentManagementComponent implements OnInit {
  ...
  stripeAccountId = computed(() => this.tenantService.tenant()?.payments?.stripeAccountId);
  stripeStatus = computed(() => this.tenantService.tenant()?.payments?.stripeStatus);
  ...
  async checkStatus() {
    ...
    try {
        const checkStripeAccountStatus = httpsCallable(this.functions, 'checkStripeAccountStatus');
  ...
  async connectStripe() {
    ...
    try {
      const createConnectAccount = httpsCallable(this.functions, 'createConnectAccount');
      const tenantId = this.tenantService.tenant()?.id;

...
export class AddAppointmentUseCase {
  ...
  async execute(appointment: Appointment): Promise<void> {
    ...
    if (appointment.phone) {
      const isBlocked = await this.blacklistRepository.isBlocked(
        appointment.phone,
      );
      if (isBlocked) {
        // Error genérico silencioso para no dar pistas
        throw new Error(
          'Error al procesar la solicitud. Por favor contacte con el establecimiento.',
        );
      }
    }
    ...
    const settings = await firstValueFrom(
      this.getBarberSettingsUseCase.execute(),
    );
